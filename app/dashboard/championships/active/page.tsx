...
import {
  ...
  Activity, Zap, Crown, Target, Timer, Globe, Dumbbell,
} from "lucide-react";
import { League, LeagueStatus } from "@/types/LeagueTypes";
import { Modal } from "@/components/ui/Modal";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { tournamentSchema, TournamentFormValues } from "@/validations/shampionship/shampionship";
...
const Page = () => {
  ...
  // Navigation par step avec trigger Zod explicite
  ...
  const regionsByCountry = regions.reduce((acc: Record<string, Region[]>, region) => {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            ...
            { label: "En Cours", value: activeLeagues, icon: Activity, color: "from-blue-500 to-cyan-500", bgColor: "bg-blue-50", textColor: "text-blue-700", borderColor: "border-blue-100" },
            ...
            <div key={stat.label} className={cn("relative overflow-hidden rounded-2xl border p-6 transition-all duration-300 bg-white hover:shadow-lg hover:shadow-slate-100 hover:-translate-y-1", stat.borderColor)}>
              ...
              <div className="relative flex items-start justify-between">
                <div>
                  ...
                  <p className={cn("mt-2 text-3xl font-bold tracking-tight text-slate-900", (stat as any).isCurrency && "text-2xl")}>{stat.value}</p>
                </div>
        ...
        <section className="flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="flex flex-1 gap-3 w-full md:w-auto">
            ...
            <div className="relative">
              <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as LeagueStatus | "all")} className="appearance-none bg-slate-50 border border-slate-200 rounded-xl pl-4 pr-10 py-3 text-sm text-slate-700 focus:outline-none focus:border-indigo-300 focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all cursor-pointer hover:bg-white">
            ...
            <div className="relative">
              <select value={filterFormat} onChange={(e) => setFilterFormat(e.target.value as any)} className="appearance-none bg-slate-50 border border-slate-200 rounded-xl pl-4 pr-10 py-3 text-sm text-slate-700 focus:outline-none focus:border-indigo-300 focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all cursor-pointer hover:bg-white">
        ...
        {shampLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-slate-500 mb-8 max-w-md">Créez votre première ligue ou tournoi pour lancer votre saison</p>
            <button onClick={() => setIsCreateModalOpen(true)} className="flex items-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-xl font-semibold shadow-lg shadow-slate-200 hover:shadow-xl hover:shadow-indigo-100 transition-all hover:scale-105">
                    ...
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          <span className={cn("inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border", status.bg, status.color, status.border)}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          ...
                          <div className="flex items-center gap-2 text-slate-600">
                            ...
                            <span><span className="font-semibold text-slate-700">{league._count?.teams || 0}</span>/{league.maxTeams || "∞"} équipes</span>
                          </div>
        ...
        {selectedLeague && (
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-4 text-sm">
              <div className="flex items-center gap-2 text-slate-600"><Calendar className="w-4 h-4 text-slate-400" /><span>{formatDate(selectedLeague.startDate)} - {formatDate(selectedLeague.endDate)}</span></div>
              <div className="flex items-center gap-2 text-slate-600"><MapPin className="w-4 h-4 text-slate-400" /><span>{selectedLeague.region?.name}, {selectedLeague.region?.country?.name}</span></div>
              <div className="flex items-center gap-2 text-slate-600"><Users className="w-4 h-4 text-slate-400" /><span>{selectedLeague._count?.teams || 0}/{selectedLeague.maxTeams} équipes</span></div>
            </div>
            {selectedLeague.totalPrizePool > 0 && (
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2"><Award className="w-4 h-4 text-amber-500" /><span className="font-semibold text-slate-800">Récompenses</span></div>
                  <span className="text-xs font-medium text-slate-500">Total: {formatCurrency(selectedLeague.totalPrizePool)}</span>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { place: "1ère", prize: selectedLeague.firstPlacePrize || 0, bg: "bg-amber-100", text: "text-amber-600", num: "1" },
                    { place: "2ème", prize: selectedLeague.secondPlacePrize || 0, bg: "bg-slate-100", text: "text-slate-600", num: "2" },
                    { place: "3ème", prize: selectedLeague.thirdPlacePrize || 0, bg: "bg-orange-100", text: "text-orange-600", num: "3" },
                  ].map(({ place, prize, bg, text, num }) => (
                    <div key={place} className="text-center p-3 bg-white rounded-lg border border-slate-200">
                      <div className={cn("w-8 h-8 mx-auto mb-1 rounded-full flex items-center justify-center", bg)}>
                        <span className={cn("text-xs font-bold", text)}>{num}</span>
                      </div>
                      <p className="text-xs font-medium text-slate-500">{place}</p>
                      <p className="font-bold text-slate-800 text-sm">{formatCurrency(prize)}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {selectedLeague.rules && (
              <div className="space-y-2">
                <h4 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><Info className="w-4 h-4 text-slate-400" /> Règlement</h4>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-100">
              <div className="text-xs text-slate-400">ID: {selectedLeague.id.substring(0, 8)}... • {selectedLeague.isPublic ? "Public" : "Privé"}</div>
              <div className="flex gap-2">
                ...
                <button className="px-4 py-2 bg-indigo-600 text-white hover:bg-indigo-700 rounded-lg text-sm font-medium transition-colors flex items-center gap-2"><Edit className="w-4 h-4" /> Modifier</button>
              </div>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} id="create-form" className="space-y-6">
          ...
          <div className="flex gap-2 mb-8">
            {[1, 2, 3, 4].map((s) => (
              <div key={s} className="flex-1 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                <div className={cn("h-full transition-all duration-500 rounded-full", s < step ? "bg-slate-400" : s === step ? "bg-indigo-600" : "bg-transparent")} />
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              {/* Nom */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Nom de la compétition *</label>
                <input {...register("name")} className={inputCls(!!errors.name)} placeholder="Ex: Coupe des Vacances Calavi 2026" />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <select {...register("format")} className={inputCls(!!errors.format) + " appearance-none cursor-pointer"}>
                      <option value="tournament">Tournoi</option>
                      <option value="league">Championnat</option>
                    </select>
                ...
                <div>
                  ...
                  <div className="relative">
                    <select {...register("category")} className={inputCls(!!errors.category) + " appearance-none cursor-pointer"}>
              ...
              <div>
                ...
                <div className="relative">
                  <select {...register("tier")} className={inputCls(!!errors.tier) + " appearance-none cursor-pointer"}>
                ...
                <FieldError msg={errors.tier?.message} />
              </div>

              {/* Région */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5 flex items-center gap-2"><Globe className="w-4 h-4 text-slate-400" /> Région *</label>
                {apiLoading ? (
                  <div className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-400 flex items-center gap-2">
                    <div className="w-4 h-4 border-2 border-slate-300 border-t-indigo-500 rounded-full animate-spin" /> Chargement...
                  </div>
                ) : (
                  <div className="relative">
                    <select {...register("regionId")} className={inputCls(!!errors.regionId) + " appearance-none cursor-pointer"}>
              ...
              {/* Sport */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5 flex items-center gap-2"><Dumbbell className="w-4 h-4 text-slate-400" /> Sport *</label>
                {apiLoading ? (
                  <div className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-400 flex items-center gap-2">
                    <div className="w-4 h-4 border-2 border-slate-300 border-t-indigo-500 rounded-full animate-spin" /> Chargement...
                  </div>
                  ...
                  <div className="relative">
                    <select {...register("sportId")} className={inputCls(!!errors.sportId) + " appearance-none cursor-pointer"}>
                ...
                <FieldError msg={errors.sportId?.message} />
              </div>

              {/* Description */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Description</label>
                <textarea {...register("description")} rows={3} className={inputCls(!!errors.description) + " resize-none"} placeholder="Décrivez votre compétition..." />
              </div>
            </div>
          )}

          {/* ── ÉTAPE 2 — Planning & Capacité ─────────────────────────────── */}
          {step === 2 && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">Équipes minimum *</label>
                  <input type="number" {...register("minTeams", { valueAsNumber: true })} min={2} className={inputCls(!!errors.minTeams)} />
                  <FieldError msg={errors.minTeams?.message} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">Équipes maximum *</label>
                  <input type="number" {...register("maxTeams", { valueAsNumber: true })} min={2} className={inputCls(!!errors.maxTeams)} />
                  <FieldError msg={errors.maxTeams?.message} />
                </div>
              </div>

              {/* Frais */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Frais d'inscription (FCFA)</label>
                <div className="relative">
                  <input type="number" {...register("registrationFee", { valueAsNumber: true })} min={0} className={inputCls(!!errors.registrationFee)} />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 text-xs font-medium pointer-events-none">FCFA</span>
                </div>
                <FieldError msg={errors.registrationFee?.message} />
              </div>

              {/* Format knockout */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Format éliminatoire</label>
                <div className="relative">
                  <select {...register("knockoutFormat")} className={inputCls(!!errors.knockoutFormat) + " appearance-none cursor-pointer"}>
              ...
              <div className="flex flex-col gap-3 p-4 bg-slate-50 rounded-xl border border-slate-100">
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  <input type="checkbox" {...register("hasThirdPlaceMatch")} className="w-4 h-4 rounded accent-indigo-600" />
                  <span className="text-sm text-slate-700">Match pour la 3ème place</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  <input type="checkbox" {...register("isPublic")} className="w-4 h-4 rounded accent-indigo-600" />
                  <span className="text-sm text-slate-700">Compétition publique</span>
                </label>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-5">
              ...
              {/* Cagnotte + podium */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Cagnotte totale (FCFA) *</label>
                <div className="relative">
                  <input type="number" {...register("totalPrizePool", { valueAsNumber: true })} min={0} className={inputCls(!!errors.totalPrizePool)} />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 text-xs pointer-events-none">FCFA</span>
                </div>
                <FieldError msg={errors.totalPrizePool?.message} />
              </div>

              <div className="grid grid-cols-3 gap-3">
                {([
                  { label: "1ère place *", field: "firstPlacePrize" as const, accent: "amber" },
                  { label: "2ème place *", field: "secondPlacePrize" as const, accent: "slate" },
                  { label: "3ème place *", field: "thirdPlacePrize" as const, accent: "orange" },
                  ...
                  <div key={field}>
                    ...
                    <input
                      ...
                      min={0}
                      placeholder="0"
                      className={inputCls(!!(errors as any)[field])}
                    />
                    <FieldError msg={(errors as any)[field]?.message} />
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                  ...
                  <input type="number" {...register("individualPrizes.meilleur_joueur_MVP", { valueAsNumber: true })} min={0} placeholder="0" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all" />
                  <FieldError msg={errors.individualPrizes?.meilleur_joueur_MVP?.message} />
                </div>
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                  ...
                  <input type="number" {...register("individualPrizes.soulier_d_or_buteur", { valueAsNumber: true })} min={0} placeholder="0" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all" />
                  <FieldError msg={errors.individualPrizes?.soulier_d_or_buteur?.message} />
                </div>
              </div>

              {/* Distribution texte */}
              <div className="space-y-3">
                <h4 className="text-sm font-bold text-slate-700 flex items-center gap-2"><Crown className="w-4 h-4 text-amber-600" /> Description des prix *</h4>
                <div>
                  <input {...register("prizeDistribution.champion")} className={inputCls(!!errors.prizeDistribution?.champion)} placeholder="Ex: Trophée + 500.000 FCFA + médailles" />
                  <FieldError msg={errors.prizeDistribution?.champion?.message} />
                </div>
                <div>
                  <input {...register("prizeDistribution.vice_champion")} className={inputCls(!!errors.prizeDistribution?.vice_champion)} placeholder="Ex: 300.000 FCFA + médailles" />
                  <FieldError msg={errors.prizeDistribution?.vice_champion?.message} />
                </div>
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="space-y-5">
              ...
              {/* Amendes */}
              <div>
                <label className="text-sm font-semibold flex items-center gap-2 text-rose-600 mb-3"><AlertCircle className="w-4 h-4" /> Amendes cartons</label>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-slate-500 mb-1.5 block">Carton jaune (FCFA)</label>
                    <input type="number" {...register("rules.discipline.yellow_card_fine", { valueAsNumber: true })} min={0} placeholder="1000" className={inputCls(!!errors.rules?.discipline?.yellow_card_fine)} />
                    <FieldError msg={errors.rules?.discipline?.yellow_card_fine?.message} />
                  </div>
                  <div>
                    <label className="text-xs text-slate-500 mb-1.5 block">Carton rouge (FCFA)</label>
                    <input type="number" {...register("rules.discipline.red_card_fine", { valueAsNumber: true })} min={0} placeholder="2500" className={inputCls(!!errors.rules?.discipline?.red_card_fine)} />
                    <FieldError msg={errors.rules?.discipline?.red_card_fine?.message} />
                  </div>
                </div>
              </div>
              ...
              {/* Logistique match */}
              <div>
                <label className="text-sm font-semibold flex items-center gap-2 text-blue-600 mb-3"><Users className="w-4 h-4" /> Logistique match</label>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-slate-500 mb-1.5 block">Durée du match</label>
                    <input type="text" {...register("rules.match_logistics.duration")} placeholder="90min" className={inputCls(!!errors.rules?.match_logistics?.duration)} />
                    <FieldError msg={errors.rules?.match_logistics?.duration?.message} />
                  </div>
                  <div>
                    <label className="text-xs text-slate-500 mb-1.5 block">Remplacements max</label>
                    <input type="number" {...register("rules.match_logistics.substitution_limit", { valueAsNumber: true })} min={0} max={11} placeholder="5" className={inputCls(!!errors.rules?.match_logistics?.substitution_limit)} />
                    <FieldError msg={errors.rules?.match_logistics?.substitution_limit?.message} />
                  </div>
                </div>
              </div>

              {/* Critères de départage (informatif) */}
              <div className="p-4 border border-dashed border-slate-200 rounded-xl bg-slate-50">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {["Face-à-face", "Différence de buts", "Buts marqués", "Fair-play"].map((c) => (
                    <span key={c} className="px-3 py-1.5 bg-white rounded-lg text-xs font-medium text-slate-600 border border-slate-200 shadow-sm">{c}</span>
                  ))}
                </div>
              </div>
            </div>
